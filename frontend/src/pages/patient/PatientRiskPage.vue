...
<template>
  <section v-if="patient" class="patient-feature-grid">
    <article v-if="activeTab === 'current'" class="clinical-card">
    ...
    <article v-if="activeTab === 'current' || activeTab === 'evidence'" class="clinical-card">
      <h2>证据摘要</h2>
      <p>{{ evidence }}</p>
    </article>

    <article v-if="activeTab === 'current' || activeTab === 'advice'" class="clinical-card">
      <h2>辅助建议</h2>
      <ol>
