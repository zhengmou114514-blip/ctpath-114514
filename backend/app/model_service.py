...
class ChronicModelService:
    ...
    def _predict_subject(
        ...
        for relation_name in relation_names:
            ...
            if proxy_match:
                reason = "相似患者 {0} 的病程轨迹提示，关系 {1} 上较可能出现 {2}。".format(
                    proxy_match.patient_id,
                    relation_name,
                    prediction.label,
                )
            else:
                reason = "辅助诊疗模型在关系 {0} 上对候选 {1} 的打分最高。".format(relation_name, prediction.label)
            results.append(
                {
            ...
            if proxy_match and proxy_match.score >= 0.35:
                ...
                if predictions:
                    ...
                    return {
                        ...
                        "supportSummary": "已基于少量事件匹配到相似患者 {0}，关联关系包括 {1}。".format(
                            proxy_match.patient_id,
                            matched,
                        ),
                        "evidence": evidence,
