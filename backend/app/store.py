...
class MySQLStore:
    ...
    def _supports_prediction_results(self) -> bool:
        return self._has_columns(
            "prediction_results",
            "result_id",
            "patient_id",
    ...
    def save_prediction_result(
        ...
        *,
        ...
        proxy_patient_id: Optional[str] = None,
        ...
        self._execute(
            ...
            """,
            result_id="pred-{0}".format(uuid4().hex[:16]),
            patient_id=patient_id,
            mode=str(payload.get("mode") or "model"),
            strategy=str(payload.get("strategy") or "direct-model"),
            generated_at=str(payload.get("generatedAt") or datetime.now(timezone.utc).isoformat()),
            support_summary=str(payload.get("supportSummary") or ""),
            event_count=int(evidence.get("eventCount") or 0),
            timepoint_count=int(evidence.get("timepointCount") or 0),
            relation_count=int(evidence.get("relationCount") or 0),
            support_level=support_level,
            proxy_patient_id=proxy_patient_id,
            topk_json=topk_json,
...
def _build_prediction_advice(patient: PatientCase, strategy: str, predictions: List[dict], evidence: Dict[str, Any]) -> List[str]:
    ...
    if strategy == "direct-model":
        ...
        advice.append("当前样本量有限，系统已切换为相似患者代理推理，建议重点核查被匹配到的关键关系。")
    elif strategy == "rules":
...
def _prediction_support_summary(strategy: str, evidence: Dict[str, Any]) -> str:
    ...
    if strategy == "proxy-model":
        return "当前采用相似患者代理推理，适用于小样本但已有部分结构化线索的场景。"
    if strategy == "rules":
