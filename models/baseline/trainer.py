...
class BaselineTrainer:
    ...
    def train_epoch(
        ...
        for i in tqdm(
            ...
            if self.num_neg > 1:
                neg_scores_tail = neg_scores_tail.view(self.num_neg, current_batch_size).transpose(0, 1)
                neg_scores_head = neg_scores_head.view(self.num_neg, current_batch_size).transpose(0, 1)
            else:
                neg_scores_tail = neg_scores_tail.view(current_batch_size, 1)
                neg_scores_head = neg_scores_head.view(current_batch_size, 1)
...
def load_data(dataset_path: str) -> Tuple[torch.LongTensor, ...]:
    ...
    if not first_token.isdigit():
        print("检测到字符串实体格式，构建ID映射...")
        return load_data_with_mapping(dataset_path)
